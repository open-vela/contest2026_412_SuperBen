...
extern int esp_timer_early_init(void); /* esp_err_t == int */
...
int esp_hr_timer_init(void)
{
  ...
  /* ESP-IDF's esp_timer requires early init first: it programs the
   * systimer hardware and seeds the system time.  Upstream ESP-IDF runs
   * esp_timer_early_init() through the ESP_SYSTEM_INIT_FN framework,
   * which is NOT executed under NuttX, so call it explicitly here.
   * Skipping it leaves the systimer uninitialised and esp_timer_init()
   * crashes inside esp_timer_impl_init().
   */

  ret = esp_timer_early_init();
  if (ret != ESP_OK)
    {
      return ERROR;
    }

  /* Initialize the timer subsystem (task + ISR) */
